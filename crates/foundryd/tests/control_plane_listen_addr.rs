...
fn reserve_non_default_port() -> u16 {
    loop {
        if let Ok(listener) = std::net::TcpListener::bind("127.0.0.1:0") {
            let port = listener.local_addr().map_or(50051, |socket_addr| socket_addr.port());
            drop(listener);
            if port != 50051 {
                return port;
            }
...
fn spawn_foundryd(home: &TempDir, listen_addr: &str) -> Result<ChildGuard, io::Error> {
        ...
        match FoundryClient::connect(client_addr.clone()).await {
            ...
            Err(error) => {
                return Err(io::Error::other(format!(
                    "foundryd never became reachable on configured addr {client_addr}: {error}"
                ))
                .into());
            }
        }
