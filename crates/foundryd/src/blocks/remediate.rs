...
mod tests {
    ...
    fn registry_with_project(name: &str, path: &str, agent: Option<&str>) -> Arc<Registry> {
        Arc::new(Registry {
            version: 2,
    ...
    async fn skips_when_main_branch_is_clean() {
        let block = RemediateVulnerability::new(Arc::new(Registry::default()));
    ...
    async fn emits_remediation_completed_when_project_found() {
        ...
        let registry = registry_with_project("my-project", "/tmp", Some("claude"));
