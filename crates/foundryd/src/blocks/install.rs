    ...
    ) -> Pin<Box<dyn std::future::Future<Output = anyhow::Result<TaskBlockResult>> + Send + '_>>
    {
        ...
        Box::pin(async move {
            ...
            let (method_name, cmd_result) = match &install_config {
                InstallConfig::Command { command } => {
                    tracing::info!(project = %project, command = %command, "running install command");
                    let project_dir = Path::new(&entry.path);
                    let result =
                        crate::shell::run(project_dir, "sh", &["-c", command], None, None).await?;
                    ("command", result)
                }
...
mod tests {
    ...
    async fn skips_when_project_not_in_registry() {
        let block = InstallLocally::new(Arc::new(Registry::default()));
