...
/// Executes the plan to fix the identified code quality issue.
///
...
impl TaskBlock for ExecutePlan {
    ...
    ) -> Pin<Box<dyn std::future::Future<Output = anyhow::Result<TaskBlockResult>> + Send + '_>>
    {
        ...
        Box::pin(async move {
            ...
            // Build gate context if available in the payload chain
            ...
            let prompt = format!(
                "Execute the following plan to improve this codebase. \
                 Make the changes described, then verify your changes compile \
                 and pass any obvious checks.\n\nPlan:\n{plan}{gates_context}"
            );
            ...
            tracing::info!(project = %project, "executing plan via agent");
            ...
            let (raw_output, exit_code, success, summary) = match response {
                ...
                Err(err) => {
                    tracing::warn!(error = %err, "agent invocation failed for plan execution");
...
mod tests {
    ...
    fn plan_event(project: &str) -> Event {
        Event::new(
            EventType::PlanCompleted,
            project.to_string(),
            Throttle::Full,
    ...
    #[tokio::test]
    async fn successful_execution_emits_success() {
        ...
        let trigger = plan_event("my-project");
        ...
        assert!(invocations[0].prompt.contains("Extract UserService"));
    }

    #[tokio::test]
    async fn failed_execution_emits_failure() {
        let dir = tempfile::tempdir().unwrap();
    ...
    async fn project_not_in_registry_returns_failure() {
        ...
        let trigger = plan_event("unknown-project");
    ...
    #[test]
    fn dry_run_emits_execution_completed() {
        let agent = FakeAgentGateway::success();
        let block = ExecutePlan::new(
            agent,
            Arc::new(Registry {
