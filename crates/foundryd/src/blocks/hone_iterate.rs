...
impl TaskBlock for RunHoneIterate {
    ...
    ) -> Pin<Box<dyn std::future::Future<Output = anyhow::Result<TaskBlockResult>> + Send + '_>>
    {
        ...
        let audit_dir = entry.audit_dir.clone();

        Box::pin(async move {
            let mut args: Vec<&str> = vec!["iterate", &agent, "--json"];

            let audit_dir_str; // extend lifetime
            if let Some(dir) = &audit_dir {
                audit_dir_str = dir.clone();
                args.extend_from_slice(&["--audit-dir", &audit_dir_str]);
            }
