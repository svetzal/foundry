...
| `"python"` | `.venv/bin/pip-audit --skip-editable --format=json` | Runs the project's own `pip-audit` from `.venv`; a project without it is reported as not scanned. `--skip-editable` leaves out the project's own install, which is not a dependency. No JSON report (for example after a PyPI error) is a scan failure, never a clean scan |
