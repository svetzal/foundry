...
> **Note for scripts/automation**: without `--offline`, online `foundry sentinel list/show/enable/disable` commands do not silently fall back and surface stable typed gRPC status errors instead. `list` and `show` render the daemon response directly, never read the client-side sentinel file, and if `FOUNDRY_SENTINELS_PATH` is absent the online path leaves it absent. If `foundryd` is not listening, all four commands fail and leave any existing client-side sentinel file untouched byte-for-byte. Online `enable`/`disable` are also persistence-atomic: if the daemon cannot save the sentinel store, the RPC returns `INTERNAL`, leaves daemon memory, scheduler-visible `list`/`show` state, scheduler notification state, and on-disk bytes unchanged, and the save path never falls back to direct destination writes or truncation.
